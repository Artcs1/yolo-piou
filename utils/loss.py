...
from utils.general import bbox_piou
from utils.general import xywh2gauss
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            if hellinger == False:
                lbox += (iou).mean()
                tobj[b, a, gj, gi] = (model.gr) + model.gr * iou.detach().clamp(0).type(tobj.dtype)
            else:
